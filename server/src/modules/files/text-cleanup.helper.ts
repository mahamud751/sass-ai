/** Repair PDF icon-font garbage and encoding issues in extracted text */

const ICON_REPLACEMENTS: [RegExp, string][] = [
  [/Ø=ÜÍ\s*/g, '📍 '],
  [/Ø=ÜÞ\s*/g, '📞 '],
  [/Ø=Üç\s*/g, '✉️ '],
  [/Ø<ß\s*[\u0300-\u036f]?\s*/g, ''],
...
export function fixMojibake(text: string): string {
  ...
  try {
    ...
    if (bad < origBad && recovered.length > 0) return recovered;
  } catch {
    /* keep original */
  }
  return text;
...
export function cleanHtmlExtracted(html: string): string {
  let out = html || '';
  for (const [pattern, replacement] of ICON_REPLACEMENTS) {
    out = out.replace(pattern, replacement);
  }
...
export function hasPdfGarbage(text: string): boolean {
  return /Ø[=<>]|Ø<ß|[\u0080-\u009F]/.test(text || '');
}

/** Remove consecutive duplicate blocks (common in PDF icon-font extraction) */
export function deduplicateRepeatedBlocks(text: string): string {
  const input = text || '';
  ...
  for (let len = Math.min(400, Math.floor(input.length / 2)); len >= minDup; len -= 10) {
    ...
    if (idx > 0 && idx < len + 40) return input.slice(0, idx).trimEnd() + input.slice(idx + block.length);
  }
