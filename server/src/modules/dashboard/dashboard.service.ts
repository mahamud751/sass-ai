...
export class DashboardService {
  ...
  private memberVisibilityWhere(ctx: Awaited<ReturnType<FamilyAccessService['getContext']>>): Prisma.FamilyMemberWhereInput {
    const or: Prisma.FamilyMemberWhereInput[] = [];
    if (ctx.isCaregiver) or.push({ ownerUserId: ctx.userId });
    if (ctx.linkedMemberIds.length) or.push({ id: { in: ctx.linkedMemberIds } });
    if (!or.length) or.push({ ownerUserId: ctx.userId });
    return { OR: or };
  }

  async getEmergencyOverview(userId: string) {
  ...
  async getDashboard(userId: string) {
    ...
    const todayDoses = todayMedicines.flatMap((med) => {
      ...
      return times.map((time) => {
        const logged = med.logs.find(
          (l) => format(new Date(l.scheduledTime), 'HH:mm') === time,
        );
        return {
          medicineId: med.id,
          name: med.name,
    ...
    if (pendingDoseCount > 0) {
      aiSuggestions.push(
        `${pendingDoseCount} medicine dose(s) due today — mark them in Medicine`,
      );
    }
    if (dueVaccines > 0) {
    ...
    if (!aiSuggestions.length) {
      aiSuggestions.push(
        ctx.isLinkedMember && !ctx.isCaregiver
  ...
  private async getFamiliesForUser(userId: string) {
