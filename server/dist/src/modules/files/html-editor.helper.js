...
exports.parseLibreOfficeHtml = parseLibreOfficeHtml;
exports.buildLibreOfficeHtml = buildLibreOfficeHtml;
exports.sanitizeLibreOfficeHtmlForBrowser = sanitizeLibreOfficeHtmlForBrowser;
exports.injectEditorChrome = injectEditorChrome;
exports.stripEditorArtifacts = stripEditorArtifacts;
exports.sanitizeEditorHtml = sanitizeEditorHtml;
exports.rewriteEditorAssetUrls = rewriteEditorAssetUrls;
function parseLibreOfficeHtml(html) {
    const source = html || '';
...
function buildLibreOfficeHtml(styles, body, title = 'Document', bodyAttrs = ' lang="en-US" dir="ltr"') {
...
function sanitizeLibreOfficeHtmlForBrowser(html, bundleBaseUrl) {
    let result = html || '';
...
function injectEditorChrome(html) {
    const chrome = `
...
function sanitizeEditorHtml(body) {
    let html = (body || '').trim();
    html = html.replace(/[\u200B-\u200D\uFEFF]/g, '');
...
function rewriteEditorAssetUrls(html, bundleId, mode) {
    const prefix = `/uploads/editor/${bundleId}/`;
    if (mode === 'absolute') {
        let result = html.replace(/((?:src|href)=["'])(?!https?:|\/|data:|#|mailto:)([^"']+)(["'])/gi, `$1${prefix}$2$3`);
        result = result.replace(/url\(\s*(['"]?)(?!https?:|\/|data:)([^'")]+)\1\s*\)/gi, `url($1${prefix}$2$1)`);
        return result;
    }
    let result = html;
    const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    result = result.replace(new RegExp(`((?:src|href)=["'])(?:https?:\\/\\/[^"']+)?${escapedPrefix}`, 'gi'), '$1');
    result = result.replace(new RegExp(`url\\(\\s*(['"]?)(?:https?:\\/\\/[^'")]+)?${escapedPrefix}`, 'gi'), 'url($1');
    result = result.replace(new RegExp(escapedPrefix, 'g'), '');
    return result;
}
//# sourceMappingURL=html-editor.helper.js.map